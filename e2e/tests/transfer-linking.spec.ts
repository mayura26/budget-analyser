...
test.describe('Transfer Linking', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    for (const name of ['Transfer Test A', 'Transfer Test B']) {
      ...
      while (true) {
        const card = page.locator('.rounded-lg.border').filter({ hasText: name }).first();
  ...
  test('categorise both transactions as Transfer', async ({ page }) => {
    ...
    await expect(toB.getByText('Transfer', { exact: true })).toBeVisible({ timeout: 10000 });
    ...
    await fromA.locator('button').filter({ hasNotText: /Link|Linked/ }).filter({ hasText: /\w/ }).first().click();
    await page.getByRole('option', { name: 'Transfer' }).first().click();
    await expect(fromA.getByText('Transfer', { exact: true })).toBeVisible({ timeout: 10000 });
  });
  ...
  test('link transfer and verify Linked badge', async ({ page }) => {
    ...
    await expect(popoverContent.getByText('Transfer from A')).toBeVisible({ timeout: 25000 });
