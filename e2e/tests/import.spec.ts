...
const monzoTransfersCsv = path.join(__dirname, '../fixtures/monzo-transfers.csv');
...
test.describe('Import', () => {
  ...
  test('navigate to transactions after import', async ({ page }) => {
    ...
    await expect(page.locator('tbody').getByText('Transfer from xx2407 CommBank app').first()).toBeVisible();
  });
  ...
  test('re-import same file shows all duplicates', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: /Import 0 transactions/i })).toBeDisabled();
  });
