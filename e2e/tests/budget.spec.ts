...
test.describe('Budget', () => {
  ...
  test('empty state shows no scheduled transactions message', async ({ page }) => {
    await page.goto('/budget');
    await page.getByRole('tab', { name: 'Schedules' }).click();
    await expect(page.getByText('No scheduled transactions yet.')).toBeVisible();
  });
  ...
  test('summary strip shows income value after creating schedule', async ({ page }) => {
    await page.goto('/budget');
    const incomeCard = page.locator('.rounded-lg').filter({ hasText: 'Expected Income' }).first();
  ...
  test('toggle schedule inactive', async ({ page }) => {
    ...
    const rentCard = page.locator('.rounded-lg').filter({ hasText: 'Rent' }).first();
