...
test.describe("Chat Categorise Dialog", () => {
  test.beforeAll(async ({ browser }) => {
    ...
    while (true) {
      const card = page
        ...
        .filter({ hasText: "Chat Test Account" })
        .first();
  ...
  test("Chat & Categorise button appears when uncategorised transactions exist", async ({
    page,
  }) => {
  ...
  test("dialog opens and AI sends first message", async ({ page }) => {
    ...
    if (!aiEnabled) {
      ...
      await expect(dialog.locator(".rounded-lg.bg-muted")).toBeVisible({
        timeout: 15000,
      });
    }
  });
